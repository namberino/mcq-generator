...
def _post_chat(messages: list, model: str, temperature: float = 0.2, timeout: int = 60) -> str:
    ...
    with open('../test/raw_data.json', 'w', encoding='utf-8') as f:
...
def save_markdown_local(file_content):
    with open('../test/extract_output.md', 'a', encoding='utf-8') as f:
...
def save_mcq_result_local(json_string):
    with open('../test/mcq_output.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(json_string, ensure_ascii=False, indent=2))
